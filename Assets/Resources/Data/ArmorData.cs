...
public class ArmorData : ScriptableObject
{
    ...
    public float bonusAttackSpeed;    // 기본 공격 속도 보너스 (신발용)
    public GameObject modelprefab;
    
    [Header("강화당 증가량")]
    public int defPerLevel = 1;     // 강화 레벨당 방어력 증가량 (헬멧용)
    public int hpPerLevel = 1;      // 강화 레벨당 체력 증가량 (갑옷용)
    public float attackSpeedPerLevel = 0.05f; // 강화 레벨당 공격 속도 증가량 (신발용)
    ...
    public float GetStat(EquipmentType type,int level)
    {
        switch (type)
        {
            ...
            case EquipmentType.Boots:
                // 공격 속도 (소수점 계산)
                return bonusAttackSpeed + (level * attackSpeedPerLevel);
        }
        // 예: 기본방어력 + (레벨 * 3)
        return 0f;
    }
